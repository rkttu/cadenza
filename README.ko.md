...
사용자 문서는 **[위키](https://github.com/rkttu/cadenza/wiki)** 에 있습니다 — 설치, 에디터 설정, SDK별 레퍼런스, 배포 레시피, 트러블슈팅. 위키 소스 리포지터리는 [github.com/rkttu/cadenza.wiki](https://github.com/rkttu/cadenza.wiki) (오프라인 편집용으로 클론 가능). 형식 명세는 [spec.md](spec.md) (한국어).
...
자주 발생하는 문제와 우회법은 위키 **[Troubleshooting](https://github.com/rkttu/cadenza/wiki/Troubleshooting)** 페이지에 모았습니다. 자주 만나는 항목:

- `#:sdk Cadenza@1.*` (wildcard) → "버전이 지정되지 않음" 오류 — `PackageReference`와 달리 MSBuild SDK는 정확한 버전 필요.
- 새 버전이 인식되지 않음 (stale NuGet cache) — NuGet HTTP 캐시에서 Cadenza 관련 항목만 선택적으로 정리.
- macOS의 `MSB3552: **/*.resx not found` — 1.0.1에서 수정.
- Windows에서 `Capture(...)` 결과 CJK / emoji 깨짐 — 1.0.4에서 수정.
- Cursor / Kiro / VS Code 포크에서 IntelliSense 미작동 — Open VSX의 community C# 확장 설치. 위키 [Editor Setup](https://github.com/rkttu/cadenza/wiki/Editor-Setup) 참고.
