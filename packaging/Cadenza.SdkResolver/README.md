...
The canonical Cadenza workflow does NOT require this package — `dotnet run app.cs` with an exact pinned version (`#:sdk Cadenza@1.0.13`) already works without any extra install. Install this resolver only if you specifically want the version-less / `@latest` shorthand in scripts where reproducibility is not critical (quick experiments, REPL-style throwaway code).
