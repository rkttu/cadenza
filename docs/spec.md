...
v0.2 변종 후보(`Cadenza.Mcp`)는 §13 참조.
...
다음은 v0.3+ 평가 대상이며, 사용자 데이터·요청 빈도에 따라 우선순위 결정. 모두 SDK 분화 4-criterion test (별도 기반 SDK / default property 세트 / 고유 tier 1 / 별도 사용자 세그먼트) 통과 후보:
...
| `Cadenza.MewUI` | SwiftUI-style 데스크톱 앱 (MewUI 1.0 도달 후) | 4 |

**v0.2는 `Cadenza.Mcp` 단독.** 위 후보들은 v0.3 sequence 진입 시점에 데이터 기반으로 1~2개 선정.
